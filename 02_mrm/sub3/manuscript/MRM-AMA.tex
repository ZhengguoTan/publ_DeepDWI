...
\begin{document}
	...
	\hl{In the area of parallel imaging and compressed sensing, 
	low rankness has been powerful prior knowledge for regularized iterative reconstruction.
	Mani et al.~\mbox{\cite{mani_2017_mussels}} developed 
	MUlti-Shot Sensitivity Encoded Diffusion Data Recovery using Structured Low Rank Matrix Completion (MUSSELS), 
	in which structural low rankness is enforced in multi-shot $k$-space. 
	...
	Hu et al.~\mbox{\cite{hu_2020_spa_llr}} developed 
	magnitude-based spatial-angular locally low-rank regularization (SPA-LLR), 
	which employs locally-low rank (LLR) regularization \mbox{\cite{zhang_2015_llr}}
	in joint $k$-$q$-space reconstruction. 
	Tan et al.~\mbox{\cite{tan_2024_naviepi}} extended LLR to 
	accelerated multi-band multi-shot multi-shell reconstruciton.
	Dong et al.~\mbox{\cite{dong_2025_nllr}} proposed to 
	better enforce the spatial-diffusion matrix low rankness 
	by stacking shuffled patches together. 
	However, low rank type iterative methods require long reconstruction time, 
	due to the computation of singular value decomposition on many local patches.}
	
	\hl{Recently, algorithm unrolling emerges as an interpretable and efficient deep learning technique 
	for signal and image processing \mbox{\cite{monga_2021_algunroll}}. 
	In the context of iterative image reconstruction, 
	algorithm unrolling naturally inherits domain knowledge, 
	i.e., the physics-based forward modelling and the data consistency term.
	Therefore, algorithm unrolling does not need to learn that domain knowledge from intensive training data. 
	Furthermore, algorithm unrolling can replace the regularization term in compressed sensing 
	with deep neural networks, acting as in implicit regularizer in inverse problems. 
	Traditional iterative image reconstruction represents the regularization term 
	as one linear transformation followed by one nonlinear thresholding function. 
	For instance, the low rankness regularization is done by 
	singular value soft thresholding (nonlinear) on Casorati matrices.
	In contrast, deep neural networks are constructed by multiple layers 
	comprising distinct linear transformations and nonlinear activation functions.
	Therefore, algorithm unrolling offers high representation power.
	There exist pioneering works that leverage algorithm unrolling 
	for accelerated parallel imaging reconstruction \mbox{\cite{wang_2016_dl,hammernik_2018_varnet,yang_2018_admmnet,aggarwal_2018_modl}}.}
	
	\hl{Algorithm unrolling has been introduced to DWI image reconstruction.
	Mani et al.~\mbox{\cite{mani_2021_qmodel}} proposed
	to learn a denoising autoencoder (DAE) model \mbox{\cite{hinton_2006_ae}} 
	from a dictionary simulated from the ball-and-stick model. 
	...
	Although promising, this method is specific to diffusion tensor models and 
	may not generalize well to DWI image reconstruction.
	Alternatively, self-supervised learning that requires neither large-scale dictionaries 
	nor high-quality fully-sampled reference data has been explored.
	Based on the zero-shot learning concept \mbox{\cite{yaman_2022_zs}}, 
	Cho et al.~\mbox{\cite{cho_2023_deepslr}} proposed to 
	learn a $k$-space regularization function for multi-shot DWI reconstruction. 
	...
	Similar to MUSSELS, this approach addresses the multi-shot EPI reconstruction problem 
	for a single diffusion encoding, 
	and does not perform joint reconstruction that explores $q$-space redundancy.
	Further, its effectiveness for high-resolution DWI is yet to be explored.}

	\hl{To achieve submillimeter isotropic DWI at clinically feasible scan and inference times, 
	...
	Moreover, building upon the foundation of noise2noise \mbox{\cite{lehtinen_2018_noise2noise,quan_2020_self2self,yaman_2022_zs}}, 
	we train the ADMM unrolling model using a single multi-band slice, 
	thereby enabling self-gated joint reconstruction with significantly reduced training time, 
	and without the need for large-scale dictionaries or extra training data. 
	...
	parameterized by $\omega$ \hl{(refer to Figure S1 in Supplementary Information)}.
	...
	\subsection{\hl{Retrospective} Study}
	...
	\hl{In addition, an ablation study that replaces the ResNet with Identity is provided in Figure S2 
	in Supplementary Information. Through this comparison, we observe effective denoising and 
	sharp diffusion-weighted contrast with the ResNet-embedded ADMM unrolling.}
	
	\begin{figure}
		\includegraphics[width=\columnwidth]{figures/fig3.png}
		\caption{\hl{Retrospective} study with the fully-sampled reference data acquired by Protocol \#1.
	...
	of the proposed \hl{self-gated} self-supervised ADMM unrolling method.
	...
	\subsubsection{Cross Subjects}
	
	\hl{We also evaluate the cross-subject generalizability. 
	Figure S3 in Supplementary Information displays the results of a model trained on one subject 
	but inferred on a different subject. 
	The model performs well and is applicable to different subjects with the same acquisition parameters.}
	...
	\begin{figure*}
		...
		\caption{Validation of the proposed self-gated ADMM unrolling reconstruction 
			...
			with navigated and \hl{self-gated} shot-to-shot phase maps, respectively.
			Comapred to MUSE, the ADMM unrolled reconstruction excels 
			in denoising while maintaining structural details.
			\hl{Self-gated} ADMM unrolling shows improved image quality in terms of tissue delineation 
	...
	On the contrary, \hl{self-gated} reconstructions show only subtle phase, 
	because shot-to-shot phase variations were estimated from the first echoes themselves.
	The reduced phase variation in the \hl{self-gated} reconstruction leads to less phase ambiguity.
	...
	the \hl{self-gated} ADMM unrolling reconstruction achieves strong denoising and resolves clear tissue details.
	
	
	\begin{figure*}
		\includegraphics[width=\textwidth]{figures/fig6.png}
		\caption{0.7~mm isotropic resolution DWI with the proposed \hl{self-gated} ADMM unrolling
	...
	\begin{figure*}
		...
		\caption{Diffusion tensor imaging (DTI) model derived colored fractional anisotropy (cFA) maps
			...
			the proposed \hl{self-gated} self-supervised ADMM unrolling reconstruction delivers clearer fiber orientations,
	...
	The proposed \hl{self-gated} self-supervised ADMM unrolling is able to resolve thin fibers within gray matters,
	...
	Furthermore, given that unrolled reconstructions require much shorter inference time 
	than conventional iterative regularized reconstructions such as compressed sensing 
	\hl{(refer to Table S1 in Supplemenetary Information)}, 
	our approach is feasible for clinical translation.
	...
	(2) As this work primarily focused on the development of \hl{self-gated} self-supervised unrolled reconstruction
