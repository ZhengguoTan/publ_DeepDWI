...
\hspace{1em} {\color{blue} We thank both editors for the constructive comments and resubmission invitation.
We have thoroughly addressed the raised critiques in this revision. Please note that because this is a resubmission and the manuscript is treated as new, we don't provide the marked version.
}
...
\begin{enumerate}
    ...
    \hspace{1em} {\color{blue} Thank you for the questions. 
    	...
    	\url{https://github.com/ZhengguoTan/DeepDWI/blob/main/examples/run_zsssl.py#L45}. For each repetion, the function `uniform\_samp` uses `torch.multinomial` to sample a new mask (\url{https://github.com/ZhengguoTan/DeepDWI/blob/main/src/deepdwi/recons/zsssl.py#L34}). 
